...
Topic0 <- "0x4cdde6e09bb755c9a5589ebaec640bbfedff1362d4b255ebf8339782b9942faa" #repay event
...
resEtherScan_logs <- GET("https://api.etherscan.io/api",
  ...
  b <- dataEtherScan_logs %>% 
    mutate(reserve = paste0("0x",substring(unlist(dataEtherScan_logs$topics)[seq(2,dim(dataEtherScan_logs)[1]*4,4)],27,67)),
           ...
           repayer = paste0("0x",substring(unlist(dataEtherScan_logs$topics)[seq(4,dim(dataEtherScan_logs)[1]*4,4)],27,67)),
    ...
    select(reserve,user,repayer,amount,contract_address,evt_tx_hash,evt_block_time,evt_block_number)
...
saveRDS(a,"C:\\Users\\Alex\\Desktop\\Crypto Jobs\\Aave analysis\\Input\\aave_v2_repay.RData")
